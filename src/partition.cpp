...
double getJaccardDistance(set<int> cEdges1, set<int> cEdges2) {
	...
	double xia = cEdges1.size() + cEdges2.size() - intersectionEdges.size();
...
//		ratio = (double) intersectionEdges.size() / (double) unionEdges.size();
		ratio = (double) intersectionEdges.size() / xia;
	}
...
cluster_info getDensityPartition(int M, int edgesNum,
	...
	map<int, int> edgeAndPartition = getEdgePartition(input_edge_partition);
	map<int, set<int>> partitionAndEdges;
	for (int i = 0; i < edgesNum; i++) {
		int partitionId = edgeAndPartition[i];
		set<int> value;
		mClusters[partitionId].insert(i);
	}
	clusterInfo.edgeAndPartition = edgeAndPartition;
	clusterInfo.mClusters = mClusters;
	return clusterInfo;
}
...
cluster_info getKMeansPartition(int M, string input_kMeansPartition,
	...
	map<int, double> edge_density_map;
	map<int, double> new_edge_density_map;
//	int edgesNum = edgesAndInfluEdges.size();
	ifstream file(input_kMeansPartition);
	map<int, set<int>> mClusters;
	map<int, set<int>> new_mClusters;
	map<int, int> edgeAndPartition;
	map<int, set<int>> edge_influEdges;
	set<int> edges;
	if (!file) {
//		for (int i = 0; i < edgesNum; i++) {
		map<int, set<int>>::iterator iter_map = edgesAndInfluEdges.begin();
		while (iter_map != edgesAndInfluEdges.end()) {
			int i = iter_map->first;
			iter_map++;
			if (edgesAndInfluEdges[i].size() > 0) { // filter some edges that can not influence other edges
...
//				update all the center edgeIds
				...
				for (iter_set_edgesCenter = edgesCenter.begin();
					...
					set<int>::iterator iter_set_edgesInCluster;
					int new_edgeIdCenter;
					double max_influScore = -1;
//					double min_influScore = 1000000.0;
					for (iter_set_edgesInCluster = edgesInCluster.begin();
							iter_set_edgesInCluster != edgesInCluster.end();
							iter_set_edgesInCluster++) {
						int edgeIdInCluster = *iter_set_edgesInCluster;
//						double temp_influScore = getEdgeInfluScore(0,
//								edge_influEdges[edgeIdInCluster],
//								edge_density_map, new_edge_density_map);
						double temp_influScore =
								edge_influEdges[edgeIdInCluster].size();
						if (temp_influScore > max_influScore) {
							max_influScore = temp_influScore;
							new_edgeIdCenter = edgeIdInCluster;
						}
...
//				update all the clusters based on the new found center edges
				edgesCenter.clear();
				edgesCenter = temp_edgesCenter;
//				set<int>::iterator iter_set_edgesCenter1;
//				for (iter_set_edgesCenter1 = edgesCenter.begin();
//						iter_set_edgesCenter1 != edgesCenter.end();
//						iter_set_edgesCenter1++) {
//					cout << *iter_set_edgesCenter1 << " ";
//				}
//				cout << endl;
				if (flag_replace == true) {
					mClusters.clear();
					...
					for (iter_set_edge = edges.begin();
							iter_set_edge != edges.end(); iter_set_edge++) {
						int edgeIdNotCenter = *iter_set_edge;
						if (edgesCenter.find(edgeIdNotCenter)
								== edgesCenter.end()) {
							double max_jaccardDis = -1;
//							double min_jaccardDis = 100.0;
							int edgeIdCenter_select;
...
//								}
		...
		cout << "The number of zero partitions: "
				<< (double) sum_zero / (double) sum_jac << endl;
//		write into files
		cout << "After mClusters.size(): " << mClusters.size() << endl;
		ofstream output(input_kMeansPartition, ios::app);
		set<int>::iterator iter_set_center;
		int total_num = 0;
		int m = 1;
		for (iter_set_center = edgesCenter.begin();
			...
			string line;
			int center = *iter_set_center;
			new_mClusters[m].insert(center);
			set<int>::iterator iter_set_edgeInCluster;
			edgeAndPartition[m] = center;
			for (iter_set_edgeInCluster = mClusters[center].begin();
					iter_set_edgeInCluster != mClusters[center].end();
					iter_set_edgeInCluster++) {
				edgeAndPartition[m] = *iter_set_edgeInCluster;
				new_mClusters[m].insert(*iter_set_edgeInCluster);
				total_num++;
				line += to_string(*iter_set_edgeInCluster) + ";";
			}
			line.pop_back();
		...
		while (getline(file, line)) {
			...
			for (int i = 0; i < cEdgesNum; i++) {
				int edgeId = stoi(line_vec[i]);
				cEdges.insert(edgeId);
				edgeAndPartition[edgeId] = line_num;
			}
//			int edgeIdCenter = stoi(line_vec[0]);
			new_mClusters[line_num] = cEdges;
			line_num++;
		}
	}

	cluster_info clusterInfo;
	clusterInfo.mClusters = new_mClusters;
	clusterInfo.edgeAndPartition = edgeAndPartition;
	return clusterInfo;
}
...
cluster_info getPartitionMinCut(int M, string input_partition) {
	...
	int edgeId = 0;
	while (getline(file, line)) {
		int partitionId = stoi(line) - 1;
		edgeAndPartition[edgeId] = partitionId;
		mClusters[partitionId].insert(edgeId);
		edgeId++;
	}
